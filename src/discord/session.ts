import { APIUser, GatewayOpcodes } from "discord-api-types/v9";
...
export class Session {
    ...
    async new(force: boolean = false) {
        ...
        // TODO: queue identify request to prevent rate-limiting.
        await this.shard.send({
            op: GatewayOpcodes.Identify,
            d: {
                shard: this.shard.settings.id,
                intents: Array.isArray(this.shard.settings.intents)
                    ? this.shard.settings.intents.reduce((intents, intent) => intents | intent, 0)
